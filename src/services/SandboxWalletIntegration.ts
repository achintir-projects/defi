...
export class SandboxWalletIntegration {
  ...
  constructor(walletApi: string = 'http://localhost:3000/api', sandboxApi: string = 'http://localhost:3000/api') {
  ...
  private initializeWebSocket(): void {
    ...
    if (typeof window !== 'undefined') {
      try {
        this.wsConnection = new WebSocket('ws://localhost:3000/api/socketio');
