...
export default function SimpleAutoNetworkSetup() {
  ...
  useEffect(() => {
    if (isConnected && currentChain !== POL_SANDBOX_CHAIN.name) {
      switchToPOLNetwork()
    }
  ...
  const switchToPOLNetwork = async () => {
    try {
      ...
      setCurrentChain(POL_SANDBOX_CHAIN.name)
  ...
  const getConnectionStatusColor = () => {
    ...
    if (currentChain === POL_SANDBOX_CHAIN.name) return 'bg-green-100'
    return 'bg-yellow-100'
  }

  const getConnectionStatusText = () => {
    if (!isConnected) return 'Not Connected'
    if (currentChain === POL_SANDBOX_CHAIN.name) return 'Connected to POL Sandbox'
          ...
          <div className="space-y-4">
            ...
            <div className={`p-4 rounded-lg ${getConnectionStatusColor()}`}>
              <div className="flex items-center justify-between">
                ...
                {isConnected && (
                  <Badge variant={currentChain === POL_SANDBOX_CHAIN.name ? "default" : "secondary"}>
                    {currentChain || 'Unknown Network'}
                  </Badge>
                )}
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-sm">
                <div className={`w-4 h-4 rounded-full flex items-center justify-center ${
                  currentChain === POL_SANDBOX_CHAIN.name ? 'bg-green-500' : 'bg-gray-300'
                }`}>
                  {currentChain === POL_SANDBOX_CHAIN.name && <Check className="h-3 w-3 text-white" />}
                </div>
            ...
            ) : currentChain !== POL_SANDBOX_CHAIN.name ? (
              <Button 
